...
DROP TABLE IF EXISTS daily_update_alt;
...
--more robust but longer initial pull
--Create table daily_update_alt as
--SELECT DISTINCT * FROM MAIN WHERE cik in (select DISTINCT cik from coinfo where ticker in (select distinct ticker from ticker_by_exchange)) and data_link not in (SELECT DISTINCT "SEC Data Link" FROM annual_formatted);
